...
{	
	...
	public static void FindAndCopySdkAapt()
	{
		// Locate aapt in SDK dir
		string searchDir = Path.Combine(GetAndroidSDKDir(), "build-tools");
		if (!Directory.Exists(searchDir))
		{
			UnityEngine.Debug.Log("build-tools folder not found in SDK folder. Falling back to packaged aapt.");
			return;
		}
		...
		{
			UnityEngine.Debug.Log("No directories found in SDK/build-tools folder. Falling back to packaged. aapt.");
			return;
		}
		
		var aaptFiles = buildToolsSubdirs.SelectMany(dir => Directory.GetFiles(dir, "aapt*"));
		if (aaptFiles.Count() == 0)
		{
			UnityEngine.Debug.Log("aapt not found in SDK/build-tools folder. Falling back to packaged. aapt.");
			return;
		}

		string selectedAapt = aaptFiles
			.OrderByDescending(f => File.GetLastWriteTime(f))
			.First();

		// Copy the found aapt to the apktool dir
		string targetAapt = Path.Combine(GetAPKToolDir(), Path.GetFileName(selectedAapt));

		if (PlayerPrefs.HasKey("CoherentUI_AaptCopied") && File.Exists(targetAapt) && File.GetLastWriteTime(targetAapt) > File.GetLastWriteTime(selectedAapt))
		{
			UnityEngine.Debug.Log("aapt in Android SDK folder is older than the packaged one; skipping copy.");
			return;
		}
		
		UnityEngine.Debug.Log("aapt that will be used: " + selectedAapt);
		File.Copy(selectedAapt, targetAapt, true);
		PlayerPrefs.SetInt("CoherentUI_AaptCopied", 1);
	}
